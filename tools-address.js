...
(function () {
  ...
  const PINYIN = {
    ...
    '民':'min','族':'zu','權':'quan','生':'sheng','治':'zhi','有':'you',
    '建':'jian','國':'guo','新':'xin','舊':'jiu','光':'guang','復':'fu','同':'tong',
    '維':'wei','合':'he','如':'ru','全':'quan','文':'wen','化':'hua','武':'wu','學':'xue','校':'xiao','院':'yuan','府':'fu','政':'zheng','公':'gong','市':'shi',
    '台':'tai','臺':'tai','基':'ji','隆':'long','宜':'yi','蘭':'lan','花':'hua','蓮':'lian','彰':'zhang','雲':'yun','嘉':'jia','屏':'ping',
    '京':'jing','蘇':'su','杭':'hang','寧':'ning','陽':'yang','陰':'yin','慶':'qing','重':'chong','汀':'ting','州':'zhou','青':'qing','夏':'xia',
    '甘':'gan','肅':'su','晉':'jin','遼':'liao','吉':'ji','黑':'hei','龍':'long','蒙':'meng','貴':'gui','桂':'gui','閩':'min',
    '秦':'qin','楚':'chu','齊':'qi','魯':'lu','吳':'wu','越':'yue','燕':'yan',
    '太':'tai','昆':'kun','明':'ming','長':'chang','春':'chun','沙':'sha','昌':'chang','徐':'xu','波':'bo',
    '介':'jie','經':'jing','健':'jian','康':'kang','年':'nian','美':'mei','逢':'feng','甲':'jia','勤':'qin','心':'xin','益':'yi','村':'cun',
    '凱':'kai','達':'da','格':'ge','羅':'luo','斯':'si','承':'cheng','松':'song','敦':'dun','富':'fu','錦':'jin','源':'yuan','辛':'xin','亥':'hai','景':'jing',
    '頂':'ding','底':'di','頭':'tou','尾':'wei','口':'kou','門':'men','關':'guan','橋':'qiao','街':'jie','路':'lu','巷':'xiang','弄':'long','段':'duan','號':'hao','樓':'lou','室':'shi',
    '里':'li','鄰':'lin','鄉':'xiang','鎮':'zhen','區':'qu','縣':'xian',
    '春':'chun','秋':'qiu','日':'ri','月':'yue','星':'xing','天':'tian','地':'di','金':'jin','銀':'yin','銅':'tong','鐵':'tie','玉':'yu','寶':'bao',
    '高':'gao','低':'di','寬':'kuan','深':'shen',
    '思':'si','想':'xiang','念':'nian','志':'zhi','成':'cheng','功':'gong','勝':'sheng','利':'li','立':'li','站':'zhan','行':'xing',
    '紅':'hong','黃':'huang','綠':'lu','藍':'lan','白':'bai','彩':'cai',
    '雙':'shuang','單':'dan','萬':'wan'
  };
  ...
  /* Pinyin a Chinese substring, using road-name dict then per-char fallback */
  function pinyinize(zh) {
    ...
    while (i < zh.length) {
      ...
      for (let len = Math.min(4, zh.length - i); len >= 2; len--) {
        ...
        for (const [k, v] of ROADS_S) {
          if (k === seg) { out += v; i += len; matched = true; break; }
        }
        if (matched) break;
      }
      if (matched) continue;
      const py = PINYIN[zh[i]];
      out += py ? cap(py) : '[' + zh[i] + ']';
      i++;
    }
    return out;
  ...
  function convert(raw) {
    ...
    if (distHit) {
      distEn = distHit.value;
      s = distHit.rest;
    } else {
      const dirM = s.match(/^(東區|西區|南區|北區|中區)/);
      if (dirM) {
        const dirMap = { '東區':'East Dist.','西區':'West Dist.','南區':'South Dist.','北區':'North Dist.','中區':'Central Dist.' };
        distEn = dirMap[dirM[1]];
        s = s.slice(dirM[0].length);
      } else {
        const g = s.match(/^([一-龥]{1,4})(鄉|鎮|區)/);
        if (g) {
          distEn = pinyinize(g[1]) + ' ' + ({ '鄉':'Township','鎮':'Township','區':'Dist.' }[g[2]]);
    ...
    // 5) Street name (name)(ordinal)?(direction)?(type)
    let streetEn = '';
    ...
    if (streetM) {
      ...
      streetEn = (pinyinize(namePart) + ordEn + ' ' + dirEn + ROAD_TYPE[type]).replace(/\s+/g, ' ').trim();
    ...
    // 8) Remainder (building name)
    let extra = s ? pinyinize(s) + ' Bldg.' : '';

    // Compose smallest → largest
    const parts = [];
  ...
  function init() {
    ...
    copyBtn.addEventListener('click', async () => {
      ...
      if (!text) return;
      const ok = await window.AppUtils.copyText(text);
      if (ok) window.AppUtils.flashCopied(copyBtn);
    });
  }
